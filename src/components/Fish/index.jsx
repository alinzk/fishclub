...
class Fish extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { contProps, imgProps } = toTweenKonvaProps(nextProps);
    this.refs.img.to(imgProps);
    this.refs.cont.to(contProps);
  }

  render() {
    const { contProps, imgProps } = toInitialKonvaProps(this.state.initialProps);
    return (
      <konva.Group ref="cont" {...contProps} >
        <konva.Image
          ref="img"
          image={img}
          { ...imgProps }
